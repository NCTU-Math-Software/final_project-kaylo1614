...
作法:用Z矩陣紀錄病毒的分布，如果感染源的位置不是死亡狀態，則在我們想要的位置放置病毒，病毒在傳播前先確定病毒所在位置有活體，如果有，則取randi(1,1)，值小於0.5則執行第一種，否則執行第二種傳播法，而如果場上有超過1/4的位置有病毒，將死亡參數調為1，只要死亡參數(kill)不為零，病毒所在位置就會變成死亡，且該位置病毒消失(Z矩陣的該位置改為0)，直到每個病毒都死光，重設死亡參數。
